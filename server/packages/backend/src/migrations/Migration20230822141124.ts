...
export class Migration20230822141124 extends Migration {
    ...
    async up(): Promise<void> {
        ...
        this.addSql('create table `run` (`id` text not null, `nfc_id` text not null, `url` text not null, `destination_ip` text null, `current_hop_index` integer not null default 1, `created_at` datetime not null, `updated_at` datetime not null, `image_path` text null, constraint `run_destination_ip_foreign` foreign key(`destination_ip`) references `address`(`ip`) on delete set null on update cascade, primary key (`id`));');
        this.addSql('create unique index `run_destination_ip_unique` on `run` (`destination_ip`);');

        this.addSql('create table `run_hop` (`id` integer not null primary key autoincrement, `address_ip` text null, `type` text check (`type` in (\'previous\', \'alternative\', \'recommended\')) not null, `terminal_id` integer not null, `run_id` text not null, `status` text check (`status` in (\'actual\', \'potential\')) not null, `hop` integer not null, `created_at` datetime not null, `updated_at` datetime not null, constraint `run_hop_address_ip_foreign` foreign key(`address_ip`) references `address`(`ip`) on update cascade, constraint `run_hop_terminal_id_foreign` foreign key(`terminal_id`) references `terminal`(`id`) on update cascade, constraint `run_hop_run_id_foreign` foreign key(`run_id`) references `run`(`id`) on update cascade);');
