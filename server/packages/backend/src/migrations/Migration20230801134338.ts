...
export class Migration20230801134338 extends Migration {
    ...
    async up(): Promise<void> {
        ...
        this.addSql('create table `run` (`id` text not null, `nfc_id` text not null, `url` text not null, `destination_ip` text, `terminal_id` integer null, constraint `run_destination_ip_foreign` foreign key(`destination_ip`) references `address`(`ip`) on update cascade, constraint `run_terminal_id_foreign` foreign key(`terminal_id`) references `terminal`(`id`) on delete set null on update cascade, primary key (`id`));');
